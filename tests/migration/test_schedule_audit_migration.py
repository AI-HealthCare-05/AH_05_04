...
def _schedule_audit_down_revision() -> str:
    migration_path = ROOT / "backend/alembic/versions/423a1b2c3d4e_schedule_audit.py"
    spec = importlib.util.spec_from_file_location("schedule_audit_migration", migration_path)
...
def test_baseline_requires_private_artifact_and_preserves_unknown_history(tmp_path: Path) -> None:
    ...
    command.downgrade(cfg, _schedule_audit_down_revision())
    ...
    try:
        ...
        with pytest.raises(RuntimeError, match="schedule_baseline_path"):
            command.upgrade(cfg, "head")
        assert asyncio.run(_run("SELECT version_num FROM alembic_version")) == _schedule_audit_down_revision()
