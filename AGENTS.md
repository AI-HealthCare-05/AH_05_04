...
- For every shared-contract change, update the relevant Markdown file under `docs/contracts/<domain>/`. If no relevant contract file exists, create one in the closest domain directory and add it to `docs/contracts/README.md`; do not use lifecycle folders such as `current/`, `targets/`, or `proposed/`, and do not duplicate a contract across domains. The absence of an existing file is not a reason to leave the contract undocumented.
