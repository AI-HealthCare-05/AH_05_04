...
- Do not introduce database triggers, row-level security policies, or stored procedures for business rules. Implement explicit Python Service/Repository validation and transactions, supported by ordinary constraints and least-privilege DB roles. Preserve applied migration history; remove legacy triggers through forward migrations. Do not expand the legacy exception manifest to permit new definitions.
