...
공유 오류 응답과 사용자 리소스 소유권·실패 상태 규칙은 [`../contracts/current/backend-common-patterns.md`](../contracts/current/backend-common-patterns.md)와 [`../contracts/current/backend-error-response.md`](../contracts/current/backend-error-response.md)를 기준으로 합니다.
...
공유 규칙은 [`../contracts/current/backend-common-patterns.md`](../contracts/current/backend-common-patterns.md) §2.2를 따릅니다. 여기서는 그 규칙의 구체적인 `flush()`/`commit()` 동작 차이와 코드 예시를 다룹니다.
