...
[Issue #306](https://github.com/AI-HealthCare-05/AH_05_04/issues/306)은 v2를 수정하지 않고 `chat-v3-history-eval-v1`을 추가했습니다. v3는 기존 11 case, 대상 불명확 처방약 사례, 단일 약물 암시 질문 회귀와 대상 불명확 상태의 현재 호흡곤란 응급 우선 사례를 포함합니다. runner의 canonical 경로·`dataset_id`·고정 SHA-256은 v3를 가리킵니다. 결정론적 replay는 기준선·history 각각 14/14, 후속 대상 식별 2/2, 안전 rule 위반 0건입니다. 응급 사례는 공백·Unicode·종결부호를 정규화한 전체 응답이 승인된 긴급 행동 문장과 일치할 때만 통과해 부정·유예·후행 상쇄 문장을 fail-closed로 거부합니다. live 모드에서는 대상 불명확 사례의 history 입력을 총 30회 호출하고 특정·재확인·복수 나열·오선택·미분류 개수만 기록하며, 원시 응답은 결과 artifact에 저장하지 않습니다. 같은 전체 응답 정규화 기준으로 승인된 고정 재확인 문장과 일치할 때만 재확인으로 집계하고, 단순 약명 언급이나 뒤따르는 복약 조언은 통과로 집계하지 않습니다. 30회 모두 재확인일 때만 반복 평가를 통과합니다.

2026-09-08 합성 OpenAI live 실행은 이전 13-case fixture에서 총 85 response를 측정했으므로 현재 14-case fixture의 근거로 사용하지 않습니다. [2026-09-09 current canonical 실행](validation/issue-306-chat-live-evaluation.md)은 dataset SHA `210c1e…`, prompt SHA `85b82c…`로 87 response를 측정해 대상 불명확 재확인 30/30, 복수 나열·오선택·미분류 0/30을 기록했습니다. 전체 14-case live gate는 baseline 9/14, history 12/14, safety rule violation count 5로 `passed=false`였으며 이 실패를 전체 품질 통과로 해석하지 않습니다. 모든 합성 Local 실행은 Production 공개나 Privacy 승인의 근거가 아닙니다.
...
- 비동기 OCR 접수에서 `PENDING` placeholder가 Publisher/Worker까지 전달되지 못해 문서를 영구히 잠그는 상황을 막기 위해, `OCR_PENDING_ACTIVE_WINDOW_SECONDS`(기본 300초)를 지난 `PENDING`은 신규 접수 차단 대상에서 제외합니다. `PROCESSING`은 시간과 무관하게 진행 중으로 유지합니다.
