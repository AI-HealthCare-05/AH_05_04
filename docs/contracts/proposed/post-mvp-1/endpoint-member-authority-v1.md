...
3. **선행 계약 상태 (Proposed Status)**:
   - 본 계약은 `PD-315-20260908`(Approved, PR #361)의 nullable operation_code 정책을 반영한 **Proposed** 계약이다. Decision 승인은 구현 증빙이 아니므로, 구현·migration·OpenAPI/DTO·자동 테스트·실행 증빙과 지정 책임 리뷰어 승인을 갖춘 별도 PR 전까지 `docs/contracts/current/`로 승격하지 않는다.
