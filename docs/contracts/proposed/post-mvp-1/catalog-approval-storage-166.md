...
- 상태: **Proposed / 일부 구현(#526 1단계)**. 아래 §3의 5개 표 중 승인 사실 3개와 조회 전용 검증기를 구현했다.
  권한 표·감사 표·운영 발급 명령·잠금 순서 재배치는 미구현이며 §8의 확인 항목이 남아 있다.

## 구현 상태 (2026-09-14, #526 1단계)

| 항목 | 상태 |
| --- | --- |
| `catalog_source_approval`·`catalog_build_approval`·`catalog_build_approval_source` | 구현 |
| `SqlAlchemyCatalogApprovalVerifier` (조회 전용 실제 검증기) | 구현 |
| `catalog_approval_permission` | 미구현 — 승인자 명단·권한 정책 미확정 |
| `catalog_approval_audit` | 미구현 — 이벤트 값·오류 코드·보존 정책 미확정 |
| 운영 발급·철회 명령 | 미구현 — 기본 만료 기간·증빙 보관 정책 미확정 |
| §4 포트에 receipt_id·schema 추가 | 미구현 — 대신 유효 승인이 둘 이상이면 임의 선택하지 않고 실패시킨다 |
| §5 같은-session 검증·잠금 순서 | 미구현 — `load_build`가 읽기 transaction을 닫은 뒤 검증하는 기존 경계를 유지한다 |

이 단계는 "저장된 승인 사실이 없으면 Catalog를 소비하지 않는다"까지만 보장한다. 철회 즉시 모든 사용이
차단된다고 주장하지 않으며, 동시 철회 경합은 §5 구현 이후에 다룬다.
