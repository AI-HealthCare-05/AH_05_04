# Source Artifact·REJECTS 보존·삭제 정책 및 cleanup 설계 초안 (#335)

- 문서 상태: **Proposed / 정책 확정 대기**. 승인된 Target 또는 현재 실행 계약이 아니다.
...
- 2026-09-08 댓글 확인 기준: 은영님·현우님 의견은 확보했고 가빈님 정책 답변은 대기 중이다.

이 문서 PR은 보존 기간·실행 담당·승인 절차를 임의로 확정하지 않는다. 코드·migration·삭제 기능·운영 설정을 변경하지 않으며, 병합만으로 #335를 종료하거나 자동 삭제·Source Runtime을 활성화하지 않는다.
...
| R5 | rollback 잔존 객체에 유예기간 필요 | 현우 | 진행 중 수집과 분리. 기간·기산점·활성 수집 판정 방법은 미확정 |
...
| R8 | 삭제 기록 append-only, 관련 provenance가 유지되는 동안 보존 | 현우 | 기술 보존 방향으로 반영. 기간·종료 이후 보존 방식은 PM·DB/보안 최종 결정 필요 |
...
| 역할 | 댓글에서 제안된 방향 | 현재 상태 |
| --- | --- | --- |
| 보존 기간·서비스 종료 정책 | PM 정책 기준, Privacy·DB/보안 검토 반영 | 가빈님 답변 대기 |
| 삭제 실행 | Backend·운영 담당 | 현우님 제안. 실제 담당자 지정 전 |
| 배치 승인 | DB·보안 검토 후 PM 또는 지정 운영 책임자 | 현우님 제안. 승인자·위임 방식 확정 전 |
| DB·보안 검토 | 참조 판정·삭제 안전성·접근 통제·감사 | 은영님이 검토 범위 확인 |
| RAG 검토 | 대상 판정 규칙·provenance 보존 | 현우님이 범위 확인. 매 배치 실행/승인자로 지정하지 않음 |
| 김지혜 | 현재 구현·영향 정리와 정책 문서 초안 | 현재 #335 역할. cleanup 코드 구현 담당으로 자동 확정하지 않음 |

수동 승인으로 시작하자는 방향은 두 의견과 부합하지만, 삭제 실행 자체의 승인이나 담당자 배정이 완료된 것은 아니다.

## 아직 확정되지 않은 정책

아래는 기존 #335 요청을 정리한 것이다. 같은 질문을 다시 댓글로 게시하지 않는다.

| ID | 결정 항목 | 아직 필요한 답 | 확인 주체 |
| --- | --- | --- | --- |
| P1 | 대상별 보존 기간 | RAW_RESPONSE/REJECTS 및 참조·미참조 객체의 기간, 유형별 차등 여부 | 가빈님 정책 결정, DB·보안/Source 검토 |
| P2 | 보존 기산점 | 수집 시각·객체 생성·Run 완료 등 어떤 사건에서 계산하는지 | PM·DB/보안, Source 영향 검토 |
| P3 | 잔존 객체 유예 | 기간과 시작 시점, 진행 중 수집의 보호 조건 | 정책·DB/Source 협의 |
| P4 | 서비스 종료 | 종료 시 일정, 종료 후 관리 주체, 최소 증빙과 보존 장소 | PM·Privacy·DB/보안 |
| P5 | 실행·승인 운영 | 실제 실행자·승인자, 주기, 승인 유효 범위·절차 | PM·운영·DB/보안 |
| P6 | 실패 처리 | 재시도 간격·한도·장기 실패 대응과 상태 보존 | 운영·DB/보안, 정책 검토 |
| P7 | 감사 기록 보존 | 관련 provenance 보존과의 연결 및 서비스 종료 이후 기준 | PM·DB/보안·RAG |
| P8 | 정본·후속 인계 | 정책 문서 위치·version·결정 근거, 코드/운영 후속 담당 | 정책 합의 후 확정 |

`retain_until`은 은영님이 제안한 구현 후보이며 확정된 컬럼이 아니다. `created_at`도 현재 존재한다는 이유만으로 보존 기산점으로 채택하지 않는다. `SKIPPED` 역시 예시 상태이며 확정 enum이 아니다.
...
| --- | --- | --- | --- |
| S1 정책·환경 확인 | 승인된 정책 version, 종류별 기간/기산점, 유예기간, 저장소 식별, 실행·승인 책임 확인 | 이번 조사의 범위 | 정책 누락·미승인, 저장소 위치 불명확, 실제 환경 실행 승인 부재 |
...
- 객체 목록만으로 RAW_RESPONSE/REJECTS 종류가 구분되지 않으면 종류별 보존기간을 임의로 적용하지 않는다. 현재 내용 주소 key에는 종류가 없고, 한 객체가 여러 종류로 참조될 가능성도 있다. 종류 판정 근거와 복수 정책의 적용 규칙은 정책·설계 확정 전 보류 사항이다.
...
감사 이력은 Source 전용이며 기존 계정 삭제 요청이나 Job 상태를 대체하지 않는다. 재시도 간격·한도·승인 재사용 조건·장기 실패 담당과 감사 기록 보존 기간은 미확정이다. 기록을 삭제하거나 수정하는 복구를 전제하지 않는다.

## 검증 시나리오

아래는 **후속 구현을 위한 테스트 명세**이며 이번 단계에서 실행한 테스트가 아니다. 기간은 승인된 정책 또는 합성 테스트 전용 입력으로 제공하고 운영 기본값을 발명하지 않는다.
...
| T08 | 보존기간/유예기간 미경과 또는 시간 근거 없음 | 삭제 호출 0건 | P1~P3 |
| T09 | 파일 생성 후 DB commit 전 수집 진행 중 | 객체 보호, 삭제 호출 0건 | R5 |
| T10 | rollback 잔존, 종류·namespace 확인, 기간 경과·전체 무참조·승인 충족 | 나머지 실행 조건 충족 시 해당 객체만 삭제 및 기록 | R1·R5, S1~S8 |
...
| T24 | 객체 종류 불명확 또는 여러 종류의 참조 공유 | 잘못된 종류의 기간으로 삭제하지 않음 | 4절·P1 |
...
| 기간 판정 | created_at 등 원시 시각 존재 | 승인된 기간·기산점·유예 정책 반영 |
...
## 최종 정책 반영 및 문서 PR 완료 점검

- [ ] P1~P8의 정책·담당·정본 위치를 결정 근거와 함께 반영
- [ ] 기술 제안 중 채택/보류 내용을 DB·Source 검토로 구분
- [ ] 필요한 코드·운영 후속 작업의 범위와 담당 확정
- [ ] 후속 이슈 생성 전 제목·범위 공유; 생성 시 #335·#165·#323에 연결
- [ ] #335 완료 기준 전체 점검. 미충족 시 Open 유지

현재 문서에는 미확정 항목과 후속 테스트 명세가 남아 있다. Draft 해제·정책 승인·이슈 종료·Runtime 활성화는 서로 별도 판단이다.
