...
- `Expand → Backfill → Dual compatibility → Verify → Read cutover → Contract` 단계를 migration PR 기준으로 나눈다.
...
| 선행 조건 | 이유 | 완료 기준 |
| --- | --- | --- |
| PROFILE SELF 소유권 전환 기준 승인 | 현재 Current 계약은 `user_id` 기반이고 목표 ERD는 `profile_id` 기반이다. 소유권 기준이 흔들리면 Job·결과·처방 version 권한 검사가 반복 수정된다. | `profile` 테이블, SELF unique, 기존 리소스 `profile_id` backfill, OCR 소유권 chain 기준 승인 |
...
| 비동기·동기 Idempotency 테이블 분리 확정 | 목표 계약은 비동기 `idempotency_record`와 동기 `sync_idempotency_record`를 분리한다. ERD가 단일 테이블이면 DDL과 구현이 충돌한다. | ERD·계약·migration 계획에서 두 저장 방식을 같은 구조로 정렬 |
...
위 항목을 blocking으로 둔 이유는 PR 1의 DDL이 한번 병합되면 이후 PR들이 그 구조를 전제로 개발되기 때문이다. `profile_id`, `domain_type/domain_id`, 멱등성 테이블, quarantine/DLQ 범위가 나중에 바뀌면 migration뿐 아니라 API 응답, Worker 메시지, 테스트 fixture까지 다시 바뀐다.
...
| 비동기 멱등성 | `idempotency_record` | 비동기 Job 접수용. 응답 snapshot 없이 `job_id`로 최신 상태 재구성 |
| 동기 멱등성 | `sync_idempotency_record` | B·C·D 동기 상태 변경용. 최초 2xx 응답 snapshot 저장 |
...
Track A는 비동기 Job 접수와 동기 상태 변경의 멱등성 저장 방식을 분리한다.

| 구분 | 테이블 | 저장 방식 |
| --- | --- | --- |
| 비동기 OCR·Guide·Chat 접수 | `idempotency_record` | target 계약의 현재 기준은 `user_id`, `operation_id`, HMAC key digest, `request_hash`, `job_id`, 만료 시각이다. 응답 body snapshot은 저장하지 않는다. |
| 동기 B·C·D 상태 변경 | `sync_idempotency_record` | parent resource scope, HMAC key, `request_hash`, 최초 2xx status와 암호화된 response snapshot |
...
비동기와 동기 멱등성을 분리하는 이유는 재응답 방식이 다르기 때문이다. 비동기 Job은 시간이 지나며 상태가 바뀌므로 최초 응답 body snapshot을 그대로 저장하지 않고 `job_id`로 최신 상태를 다시 만든다. 반면 동기 상태 변경은 이미 완료된 2xx mutation의 결과를 같은 body로 재현해야 하므로 snapshot 저장이 필요하다.
...
| --- | --- | --- |
| 1. PROFILE 선행 | 본인 단일 SELF profile 도입, 기존 리소스 `profile_id` backfill, 소유권 조회 전환 | Contract 전에는 기존 read 경로 rollback 가능. Contract 후에는 forward-fix 우선 |
| 2. Expand | `ai_job`, `outbox_event`, `message_quarantine`, `dlq_outbox_event`, `idempotency_record`, `sync_idempotency_record`, `prescription_version`, `prescription_version_medication` 생성. 기존 테이블에 nullable FK 추가 | 신규 경로 미사용 시 nullable 컬럼·신규 테이블 제거 가능 |
| 3. Backfill | 기존 처방마다 version 1 생성, 약물 snapshot 복사, `active_version_id` 채움 | 재실행 가능한 batch는 rollback이 아니라 resume/recovery로 처리 |
| 4. Dual compatibility | version FK가 있으면 신경로, 없으면 기존 경로 fallback. 신규 write는 version snapshot과 기존 read 호환 필드 유지 | 애플리케이션 코드 rollback 가능 |
| 5. Verify | orphan, 중복 version, 무효 active pointer, snapshot 수와 핵심 값 불일치 검증 | 실패 시 cutover 금지. backfill 재개 또는 forward-fix |
| 6. Read cutover | 조회와 하위 결과 생성을 `prescription_version` 기준으로 전환 | 한 배포 구간 관찰. 문제 시 contract 전 코드 rollback |
| 7. Contract | 임시 nullable 제거, NOT NULL·FK·unique·CHECK 최종화 | 이후 destructive downgrade 금지, forward-fix 원칙 |
...
## 8. Backfill, rollback, resume/recovery 구분
...
| PR 2 | Prescription Version backfill·검증 SQL·테스트 | PR 1 | DB, 계약 |
| PR 3 | 공통 Job 접수·상태 조회 API와 Outbox 연결 | PR 1 | Backend/API, Frontend polling, Worker |
| PR 4 | 비동기·동기 Idempotency 로직 적용 | PR 1, PR 3 | Backend/API, Frontend 중복 요청 |
| PR 5 | Prescription Version 활성화 transaction과 기존 Job `STALE` 처리 | PR 2, PR 3 | Backend, OCR, Guide/Chat |
| PR 6 | OCR `ai_job_id` mapping·검증 기준 및 적용 | OCR mapping 기준 승인 | OCR, Worker, DB |
| PR 7 | Guide·Chat 비동기 연결 | PR 3 이후 | Guide/Chat, RAG/Safety, Frontend |
| PR 8 | Read cutover + Contract | 선행 PR 안정화 | 전원 담당 영역 최종 확인 |
...
- PR 1~3에서 공통 Job 접수·상태 조회·Outbox 인터페이스가 고정되면 B·C·E·F는 mock/fake adapter로 병렬 개발할 수 있다.
...
- 비동기·동기 Idempotency 분리 기준이 PR 1 차단 조건으로 명시되어 있다.
