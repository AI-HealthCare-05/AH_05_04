...
현재 이 경로나 seed 파일을 생성하지 않는다. 운영 DB에서 수동 수정하는 설정 테이블도 만들지 않는다.
...
위 항목은 테스트 계획이며 passed 수치가 아니다. 이 PR은 문서 제안만 추가하고 코드·설정 seed·DB·Frontend를 변경하지 않는다.
