# MFDS 공식 의약품 식별·Candidate 계약 v1
...
사용자가 검수·확정한 Prescription Medication을 공식 MFDS 제품 Identity 후보에 연결하고, 단일 후보 확인 결과를 append-only Identification으로 저장한다. 이 파일은 Approved Contract Freeze v4의 공식 의약품 식별 목표와 RAG-00에서 추가된 Candidate 공유 API·DTO·상태·오류 경계를 하나의 저장소 Target으로 통합한 정본이다. 별도 `medication-candidate-identification-v1` Target을 만들지 않는다. Chat Job 생성 시점은 [RAG Runtime v1](./rag-runtime-v1.md)의 2단계 Context 계약을 따르며, 자동 Guide의 모든 활성 약제 `MATCHED` 조건은 유지한다.
...
Revision `169a1b2c3d4e`에서 `prescription_version_medication` 최소 DB 기반을 만들었고 `169b2c3d4e5f`에서 기존 처방 Version 1 backfill과 신규 확정 dual-write를 연결했다. 기존 Candidate 경로는 아직 legacy Medication을 소비하는 호환 구간이므로 FK만 먼저 강제하지 않는다. #169 PR 3은 PR 2 완료 뒤 구 writer rollback으로 누락된 Version이 있을 수 있음을 전제로 같은 검증 규칙으로 먼저 재-backfill한다. 그 뒤 v2 생성과 publication gate 개방 전에 placeholder가 가리키는 legacy Medication을 `(prescription_id, display_order)`로 같은 Prescription의 Version 1 Medication에 일대일 재매핑하고, 누락·중복·값 불일치 0건을 검증한 뒤 Candidate Search·Identification FK와 read cutover를 같은 migration·contract test에서 적용한다. 검증할 수 없는 기존 행은 추정 연결·삭제하지 않고 전체 migration을 중단한다. Version 간 계보 Key는 이 v1 이관용 매핑에서 추론하지 않으며 별도 승인 전까지 추가하지 않는다.
...
Candidate Search 생성에는 `Idempotency-Key`를 요구하지 않는다. 기존 [멱등성 계약 v1](./idempotency-v1.md)의 Track F 범위는 사용자 확인·거절에만 적용한다. Backend는 약제의 확정 `medication_name`과 nullable `strength_text`를 서버에서 읽으며 클라이언트가 약품명·함량을 덮어쓰게 하지 않는다.
...
- 모든 잠금은 [처방 버전 계약의 전역 순서](./prescription-version-v1.md#동시-수정)인 `PRESCRIPTION → CHAT_SESSION(해당 시) → AI_JOB(해당 시) → 도메인 row → OUTBOX(해당 시)`를 따른다. 각 Transaction은 실제로 변경하는 row만 잠그며 역순 잠금을 금지한다.
...
`EXT-SOURCE-001`, `EXT-PRIV-001`과 관련 Safety 회귀, Source Snapshot 검증·rollback 증빙 전에는 실제 사용자 Identity와 `PUBLIC_TRACK_F`를 활성화하지 않는다. 합성 fixture를 사용하는 접근 통제 로컬 데모와 Production 공개를 구분한다.
