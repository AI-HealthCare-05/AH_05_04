# Track B Notification 계약 v1

| 항목 | 값 |
| --- | --- |
| 문서 상태 | Current |
| 구현 상태 | #430 Backend·OpenAPI 병합, #202/#474 occurrence 약 조회 인계, #535/#579 Frontend 소비 반영 |
...
[Check-in 목표 계약](../targets/post-mvp-1/checkin-v1.md), [처방 버전 목표 계약](../targets/post-mvp-1/prescription-version-v1.md), [멱등성 목표 계약](../targets/post-mvp-1/idempotency-v1.md)을 따른다. 아래 enum·필드·API·조건은 현재 Backend DTO·OpenAPI·구현 및 자동 테스트가 사용하는 계약이다.
...
`NotificationResponse`의 필수 필드는 `id`, `occurrence_id`, `occurrence_local_date`, `kind`, `scheduled_at`, `status`, `delivered_at`, `read_at`이며 read_at은 nullable이다. API 시각은 UTC RFC3339, status는 목록·읽음 응답에서 `DELIVERED`다. `occurrence_local_date`는 서버가 연결된 원본 occurrence의 `scheduled_local_date`를 `YYYY-MM-DD`로 반환한다. 알림의 `scheduled_at`은 재알림 시각일 수 있으므로 Frontend는 그 값에서 occurrence 조회 날짜를 추정하지 않는다.
...
3. 같은 `occurrence_id`로 `GET /api/v1/medication-occurrences/{occurrence_id}/medication`을 호출한다. 날짜별 occurrence와 약 응답의 `occurrence_id`, `prescription_version_id`, `prescription_version_medication_id`가 모두 일치할 때만 표시한다.
4. 날짜 조회에서 occurrence를 찾을 수 없으면 최신 처방의 비슷한 약으로 대체하지 않고 ‘관련 기록을 조회할 수 없음’ 상태를 표시한다. 이는 Check-in 없음이나 미복용을 의미하지 않는다. 비활성 version의 전달된 알림도 동일한 historical identity 경로로 조회한다.

예를 들어 원본 occurrence가 KST 9월 10일 23:00이고 재알림이 9월 11일 01:00이면 조회 날짜는 `2026-09-10`이다. UTC 날짜나 알림 날짜를 사용하는 fixture는 실패해야 한다. #202/#474 합성 fixture와 #421 browser E2E가 이 경로를 검증한다.

### 읽음

`PATCH /api/v1/notifications/{notification_id}/read`, body `{}`.

처음 요청에 서버 시각을 read_at에 저장하고 이미 읽은 경우 최초 read_at을 유지한다. 자기 소유 `DELIVERED`만 허용하며 그 외에는 공통 404다. 성공은 `200 {data: NotificationResponse}`다. `Idempotency-Key`를 요구하고 동기 snapshot을 저장하며 parent resource는 notification_id다.
...
소유권 확인·멱등 replay 후 신규 요청의 검증 순서는 다음과 같다.

1. 취소된 occurrence: 기존 `409 OCCURRENCE_CANCELLED`.
2. PENDING이 아니거나 확인 기한 경과: `409 REMINDER_NOT_ALLOWED`.
3. 기존 REMINDER가 PENDING: 기존 `409 REMINDER_ALREADY_SCHEDULED`.
4. 기존 REMINDER가 DELIVERED/CANCELLED: `409 REMINDER_LIMIT_REACHED`.
5. 시간 범위·timezone 없는 시각·추가 필드 위반: 기존 `422 VALIDATION_FAILED`.

위 오류 코드는 공통 오류 문서·OpenAPI·테스트에 반영되어 있다. PENDING 전용 조건은 occurrence 단위 재알림에 적용한다.
...
남한솔은 PR #415 [최종 Frontend 재리뷰](https://github.com/AI-HealthCare-05/AH_05_04/pull/415#pullrequestreview-5165301643)에서 이 제품 결정을 두 문서에 동일하게 반영하는 것을 전제로 추가 Frontend blocker가 없다고 확인했다. 이후 #202/#474와 #535/#579가 일정·원래 약 조회·Frontend handoff를 구현했고 #421 closeout E2E가 mutation 분리를 검증한다.
...
| 일정 PUT/PATCH | #202 취소 접점에서 동일 transaction 연결 |
| Check-in·deadline | 상태 재검증 및 occurrence 잠금으로 게시 경합 검증 |
| OpenAPI·docs/api.md·docs/data-schema.md·docs/testing.md | 병합된 DTO·runtime과 정합화 |

## 회귀 검증 시나리오

아래 시나리오는 #430 Backend 계약 테스트와 후속 #202/#474/#421 통합 검증의 회귀 기준이다.
...
- Track C NOT_TAKEN → FORGOT → REMINDER_SETUP은 Notification handoff에서 자동 실행하지 않는다. Track C 자체의 일정 설정 UI는 별도 범위다.
...
이 절은 develop에 병합된 PD-203 runtime 명세다. Production 운영 활성화와 외부 발송 채널 승인은 별도다.

- ORM: `NotificationRecord`, migration `203a1b2c3d4e`(base `423a1b2c3d4e`). 필드는 위 표와 일치하며 DB의 UUID 저장은 기존 `UUIDChar`/CHAR(36) 패턴을 따른다.
...
- 목록·읽음의 타인/없는/미전달 알림은 `404 NOTIFICATION_NOT_FOUND`, 재알림의 타인/없는 occurrence는 기존 `404 MEDICATION_OCCURRENCE_NOT_FOUND`를 사용한다. 두 오류는 존재 여부를 구분해 노출하지 않는다.
