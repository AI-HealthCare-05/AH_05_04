...
`CHAT_HISTORY_CONTEXT_ENABLED`의 기본값은 `false`이며 Staging에서는 활성화가 금지된다(Config validation 거부). 현재 Production에서도 외부 전송 승인 게이트 `EXT-PRIV-003` 및 Issue #838 Proposed Decision 검토 중으로 활성화할 수 없으며 배포 스크립트에서 차단된다. 프롬프트는 flag와 관계없이 현재 runtime 정본인 `chat-prompt-v6` 하나만 사용하고 Provider payload에 `history` 배열을 항상 포함한다. `false`이면 history를 조회하지 않고 빈 배열(`history: []`)을 전달하며, Local에서 `true`이면 허용된 최근 완료 대화를 배열에 채운다. question, history, medications 내부 문자열은 모두 지시가 아닌 JSON 데이터로 취급한다. `chat-prompt-v5`는 과거 실행 재현을 위한 historical snapshot이며 runtime에서 선택되지 않는다. 향후 승인 완료 후 Production explicit opt-in이 허용되더라도 동일 세션 격리와 `false` 롤백(FastAPI 재생성) 절차가 유지된다.
