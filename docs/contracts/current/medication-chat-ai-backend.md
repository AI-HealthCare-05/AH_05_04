...
v5 프롬프트는 병용 답변 뒤 `파라시타몰은?`을 외부 비교 대상 교체로, 중단 전 상담 안내 뒤 `왱?`을 직전 안내의 이유 질문으로 연결하도록 지시한다. 약명 구어체 해석은 공식 제품·제형 Identity 확정이 아니다. 입력에 적격 상호작용 근거가 없으므로 첫 질문과 후속 질문 모두 병용 안전성을 확인했다고 단정하지 않도록 지시한다. 이 동작은 프롬프트 지침이며 결정적 안전 검증이나 실제 모델 평가 통과를 의미하지 않는다. 기존 응급 우선순위와 고정 안전 문구, API·DTO·history 전송 경계는 유지한다.
