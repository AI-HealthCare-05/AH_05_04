...
- [RAG Answer Quality Metric·Variant 계약 v1 (#159)](./targets/post-mvp-1/rag-answer-quality-metrics-v1.md): PR #475 책임 리뷰 승인으로 확정된 Approved Target. `REQUIRED_CLAIM_RECALL`·`COMPLETENESS` 순수 DEV kernel과 manifest routing은 구현했고, Schema Set 1.5 Candidate에서 human judgment·approval·3-pair comparison-set 입력 schema를 추가했다. Human judgment 소비, `ANSWER_CORRECTNESS`·`RELEVANCE`, 실제 3-pair builder·Variant 실행은 후속 구현이며 HOLDOUT·Release 승인은 별도 게이트.
