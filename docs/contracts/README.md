...
- [Guide Runtime Preflight 계약 v1 (#180)](./proposed/post-mvp-1/guide-runtime-preflight-v1.md): #707 RAG-15 Formal Approval Pack을 소비해 공식 승인된 runtime candidate와 실제 구성된 Generator provenance·Guideline Policy·Fallback Set이 exact-match하는지 Guide 생성 호출 이전에 판정하는 순수 fail-closed preflight 계약. Phase는 request shape → approval pack consumability → generator identity → policy → fallback set 순으로 fail-fast하며 결과는 typed READY/BLOCKED다. READY는 RAG-15 static runtime candidate 승인·결속만 의미하고 Guide evidence·Card·Citation·Release·공개 준비를 의미하지 않는다. 순수 계약 커널 구현 브랜치 검토 중; Generator 실행, Evidence Handoff 조립, assessment/eligibility 권위(#712), Citation Authorization, Release Gate, 영속화, 공개 활성화는 미구현.
- [REQUEST Authority Persistence 계약 v1 (#713)](./proposed/post-mvp-1/request-authority-persistence-v1.md): #709가 차단 사유로 확인한 REQUEST Guard·Source Decision·Member Decision의 historical request-bound persistence. migration `713a1b2c3d4e`의 typed 3표, canonical JCS projection 기반 immutable artifact identity, append-only writer와 exact `ImmutableArtifactRef` 조회 primitive를 확정한다. `catalog_source_approval`·CURRENT Snapshot·`runtime_guard_decision_ref` 계열은 REQUEST authority가 아님을 함께 고정한다. 구현·로컬 검증 완료·담당 리뷰 대기이며, #709 Production Reader는 이후 연결되었고 #180 런타임 오케스트레이션은 여전히 미구현.
- [Per-request REQUEST Guard Runtime Binding 계약 v1 (#806)](./proposed/post-mvp-1/request-guard-runtime-binding-v1.md): evaluator-issued request identity, 실제 PASS/FAIL, #810 environment, Runtime Bundle exact pair, historical scope tuple/hash, 기존 #713 Guard exact binding을 append-only로 보존하고 AI Worker의 exact read/기존 Citation binding assembly seam을 정의한다. #713/#709 semantics 변경, Citation Source/Member Decision, Receipt, Guide runtime orchestration, Release Gate, `PUBLIC_TRACK_F`는 포함하지 않는다. 구현 초안·로컬 검증 진행 중이며 PostgreSQL/CI/reviewer 증빙 대기.
- [Canonical Evaluation Guard Evidence 계약 v1 (#162)](./proposed/post-mvp-1/evaluation-guard-evidence-v1.md): Run 수준 `EVALUATION_CANDIDATE` Guard, Case 수준 `EVALUATION_REQUEST` Guard, Candidate Bundle identity, FROZEN Dataset Required Case 전수 결속(exact-set coverage) 및 canonical 해시 레시피를 정의하는 Phase A1 계약 동결 후보. Proposed 상태이며 actual HOLDOUT 실행, Provider 호출, paired artifact(Phase B), Release Gate PASS, Bundle READY/Active, `PUBLIC_TRACK_F`는 미구현.
- [PATIENT_CITATION Production Source Use Approval Authority 계약 v1 (#807)](./proposed/post-mvp-1/source-use-approval-807.md): `Source Snapshot + source_code/source_version + canonical environment + SourceUsePurpose + explicit approval_version`을 exact identity로 보존하는 historical Source Use Approval authority. `PATIENT_CITATION`을 business target으로 하고 `RETRIEVAL`·Bundle membership·Snapshot CURRENT·Citation Member Decision을 대체하지 않으며, half-open validity와 one-way revocation, exact read-only AI Worker seam, 기존 Source Writer 최소 ACL을 확정한다. migration `807a1b2c3d4e` 구현·로컬 계약 검증 완료; PostgreSQL/CI/reviewer 증빙과 Current 승격은 별도.
- [Runtime Bundle PATIENT_CITATION Approval Pin v1 (#853)](./targets/post-mvp-1/runtime-bundle-citation-approval-pin-v1.md): #850 Option A에 따라 exact #807 approval을 전용 immutable Bundle child row로 pin하고 manifest projection v2, persisted-state hash recomputation, equality-only Worker read seam을 정의한다. NEW C·Citation Decision/Receipt·finalizer·`PUBLIC_TRACK_F`는 포함하지 않는다.
- [Citation Authorization Authority v1 (#869)](./targets/post-mvp-1/citation-authorization-authority-v1.md): #806 exact request replay, REQUEST Source/Member binding coordinate, #853 pin, #807 exact approval, current Source/Snapshot/Member eligibility를 순서대로 결속해 request-bound Source/Member Decision과 `CitationAuthorizationReceipt`를 4개 append-only 표에 원자적으로 저장·exact-read한다. 동일 `request_sha256` 재시도는 persisted aggregate를 canonical ref와 pure verifier로 재검증해 historical replay하며 finalizer·Release Gate·`PUBLIC_TRACK_F`는 포함하지 않는다.
- [Assessment·Eligibility Authority Persistence 계약 v1 (#712)](./proposed/post-mvp-1/assessment-eligibility-authority-persistence-v1.md): `PD-722`가 확인한 다섯 authority 값(`eligibility_receipt_ref`·`assessment_artifact_ref`·`verifier_artifact_ref`·`assessment_valid_from`·`assessment_valid_until`)의 부재를 해소하는 selected hit 단위 historical persistence. migration `712a1b2c3d4e`의 `rag_evidence_authority` 한 표, canonical JCS projection 기반 immutable artifact identity, half-open 구간 DB CHECK, writer-owned verifier identity와 append-only writer, 후속 Reader가 쓸 exact lookup key 2종을 확정한다. Catalog 전용 만료일과 CURRENT Snapshot 상태는 runtime authority가 아님을 함께 고정한다. 구현·로컬 검증 완료·담당 리뷰 대기이며, Assessment·Eligibility Authority Reader(#746)와 `GuideEvidenceHandoffRequest` authoritative assembly(#760)는 이후 연결되었고 #180 런타임 오케스트레이션은 여전히 미구현.
- [Authoritative Guide Evidence Handoff Assembly 계약 v1 (#760)](./proposed/post-mvp-1/authoritative-guide-evidence-handoff-assembly-v1.md): #709/#697/#715가 확정한 `HydratedGuideRetrievalSelection`과 #712/#746이 남긴 `PersistedEvidenceAuthority`를 pure in-memory exact binding으로 결속해 기존 `GuideEvidenceSelectionRequest`/`GuideEvidenceHandoffRequest`를 구성하고 기존 `build_guide_evidence_handoff()`에 위임하는 assembly 계약. `PersistedRetrievalRunReceipt`를 함께 입력으로 받아 기존 `compute_receipt_hash()`로 자기무결성을 재검증하고 retrieval run·chunk·snapshot/member·source/version·content를 exact 결속하며, evidence key는 같은 persisted index-member row에서 production provenance로 투영된 값을 그대로 소비한다. assessment validity 판정과 handoff hash는 기존 kernel이 계속 소유하고 거부 시 원본 `GuideEvidenceHandoffBuildOutcome`을 보존한다. DB I/O·persistence 없음. 구현·로컬 검증 완료·담당 리뷰 대기이며, Generator·Citation Authorization·#180 런타임 오케스트레이션·`PUBLIC_TRACK_F`는 미구현.
- [RAG-15 Production Guideline Evidence Input 계약 v1 (#774)](./proposed/post-mvp-1/guideline-production-evidence-input-v1.md): #760 `VerifiedGuideEvidenceHandoff`를 RAG-15 Generator/Card의 production evidence 정본으로 확정하는 입력 계약. `VerifiedGuideEvidenceSelection` 31개 필드 중 실제 소비하는 12개만 순수 투영하는 `project_guideline_evidence_from_handoff()`, explicit projection version `guideline-production-evidence-selection-v1`의 RFC 8785 JCS canonical projection, raw `content_text`를 제외하고 `content_sha256`으로만 본문을 결속하는 `compute_production_guideline_evidence_selection_hash()`, 그리고 `ApprovedGuidelineEvidenceBinding.selection_projection_sha256`의 production 의미를 확정한다. legacy `EvidenceGateOutcome`/`GatePassedKnowledgeEvidenceSelection`/`canonical_gate_selection_hash()` production dependency는 제거됐고 `handoff → EvidenceGateOutcome` converter는 만들지 않는다. Citation source identity는 `source_snapshot_ref` artifact 참조 대신 production Source 좌표를 쓴다. DB I/O·network·clock·authority 재판정 없음. 구현·로컬 검증 완료·담당 리뷰 대기이며, Dynamic Guideline Evidence Binding Authority·binding issuer/persistence·`GuidelineApprovalVerifier` production adapter·Generator orchestration wiring·Citation Authorization·Release Gate·`PUBLIC_TRACK_F`는 미구현. 세 evidence 상태(`EVIDENCE_INSUFFICIENT`/`EVIDENCE_CONFLICTED`/`EVIDENCE_STALE`)의 공개 fallback 사영 책임자는 미정(UNRESOLVED).
- [Dynamic Guideline Evidence Binding Authority 계약 v1 (#781)](./proposed/post-mvp-1/guideline-evidence-binding-authority-v1.md): #774 `ProductionGuidelineEvidenceSet`·pinned `MedicationIdentityRef[]`·`GuidelineCardDraft`에서 요청 단위 `ApprovedGuidelineEvidenceBinding[]`을 결정론적으로 derivation하고, #729 READY의 static policy/fallback pin과 그 dynamic binding ref를 기존 `GuidelineApprovalVerifierPort`에 연결하는 순수 request-scoped authority 계약. binding은 실제 draft claim/citation 조합에 대해서만 만들고 Cartesian product를 만들지 않으며, citation source 좌표 7개 전체 exact-match·pinned medication exact membership·binding identity `(evidence_key, medication_identity, scope)` 중복을 fail closed한다. binding artifact identity(`guideline-evidence-binding` / `guideline-evidence-binding-v1`)는 caller가 선택할 수 없고, verifier는 issuer가 반환한 binding tuple을 whitelist로 신뢰하지 않고 authoritative inputs에서 expected ref를 독립 재계산하며, `approval_pack_ref`/`candidate_ref`는 승인 대상이 아니다. verifier identity는 기존 `compute_verifier_artifact_ref()` convention과 동일한 canonical contract projection JCS SHA-256이다. DB·network·clock·async·persistence 없음. 구현·로컬 검증 완료·담당 리뷰 대기이며, #729 READY는 sequencing root of trust일 뿐 위조 불가능한 capability가 아니다(UNRESOLVED). Generator orchestration wiring·binding persistence·Citation Authorization·Release Gate·`PUBLIC_TRACK_F`는 미구현.
- [Guide Generator → Guideline Card Orchestration 계약 v1 (#787)](./proposed/post-mvp-1/guide-generation-card-orchestration-v1.md): #180 Slice 1 `READY_FOR_GENERATION`을 소비해 #774 production evidence 투영 → `GuidelineGenerationRequest` → `generator.generate()` 1회 → #781 dynamic authority(또는 generation failure용 static policy/fallback 승인 bridge) → `finalize_guideline_card()`까지 연결하고 `GuidelineCardOutcome`에서 멈추는 orchestration 계약. Slice 1 semantics는 불변이며 pre-generation stop(REQUEST 무효·#729 BLOCKED·#760 REJECTED)에서 Generator 호출은 0이다. policy·fallbacks·provenance를 재입력받지 않고 #729가 승인한 정본만 쓰며, `GuidelineGenerationFailure`는 재해석 없이 기존 finalizer 매핑으로 그대로 전달하고 fallback을 선택·재매핑·생성하지 않는다. #781 authority 실패는 finalizer 호출 없이 fail closed한다. `COMPLETED`/`STOPPED`는 orchestration-local 상태이며 공개 `PASS|LIMITED|REJECTED|STALE` 사영을 만들지 않는다. 구현·로컬 검증 완료·담당 리뷰 대기이며, Citation Authorization·Release Gate·Card persistence·DB/migration·Worker·Guide API·`PUBLIC_TRACK_F`는 미구현.
- [Guide Card → Claim/Citation Validation 계약 v1 (#794)](./proposed/post-mvp-1/guide-claim-citation-validation-v1.md): #787 `GuideGenerationCardOutcome`을 유일한 입력으로 소비해 GENERATED Guideline Card만 `ClaimCitationCandidateSet`으로 투영하고, claim 단위 support assessment artifact와 `ClaimSupportVerificationReceipt[]`를 만든 뒤 기존 RAG-16 `validate_claim_citations()`를 그대로 호출해 `ValidatedCitationSelection`에서 멈추는 계약. Card citation은 `upstream_outcome.ready_inputs.evidence_handoff`의 `VerifiedGuideEvidenceSelection`과 evidence key·snapshot·member·source code/version·locator·content digest 및 assessment/eligibility/retrieval/verifier 4개 authority ref까지 11개 전체 exact-match해야 하며 하나라도 어긋나면 fail closed한다. `SourceExecutionProvenance`는 #774 production evidence가 의도적으로 버린 audit field이므로 상류 handoff에서만 projection하고 새 Decision ref나 Source Snapshot artifact를 합성하지 않는다. 한 Claim이 여러 Citation을 가질 수 있으므로 임의 citation의 `assessment_artifact_ref`를 claim support로 대표시키지 않고, 모든 citation을 결속한 claim 단위 `guideline-claim-support-assessment` artifact를 만들며 Receipt는 citation이 아니라 claim당 정확히 1개다. support verifier(`guideline-claim-support-verifier`)와 validator policy(`guide-claim-citation-validator-policy`) identity는 기존 verifier identity convention과 같은 canonical contract projection JCS SHA-256이고 request-independent이며 caller가 선택할 수 없다. 기존 validator의 판정·reason은 재매핑하지 않는다. DB·network·clock·async·persistence 없음. 구현·로컬 검증 완료·담당 리뷰 대기이며, Citation Authorization·PATIENT_CITATION Guard·Citation Finalizer·Release Gate·persistence·`PUBLIC_TRACK_F`는 미구현.
...
- [Guide Generation Canonical Gap Closure v1 (#180)](./targets/post-mvp-1/guide-generation-canonical-gap-closure-v1.md): Guide topology를 동결한다. freshness는 standalone으로 유지하고 product-safety overlay는 Current MVP에서 retire하며, selection·P0 conflict authority는 retrieval/evidence boundary에 merge한다. Patient Context Card exclusion/limitation filter는 compose 뒤 standalone으로 남지만 typed carrier가 없어 `MISSING_SEMANTIC_CALLABLE`이다. #774는 projection-only다.
- [Guideline Card typed port 계약 v1](./targets/post-mvp-1/guideline-card-v1.md): production Guide evidence·PD-362 Source eligibility와 RAG-16 사이의 Request·Outcome·승인 verifier·fallback 계약 — RAG-15 persistence-free kernel 구현 검토 중(#179, PR #414), Current 아님. #774에서 production 입력이 `EvidenceGateOutcome`에서 `ProductionGuidelineEvidenceSet`으로 이동했고 입력 필드·projection·hash 의미는 [RAG-15 Production Guideline Evidence Input 계약 v1](./proposed/post-mvp-1/guideline-production-evidence-input-v1.md)이 소유한다
- [RAG Evaluation·Release Gate 계약 v1](./targets/post-mvp-1/rag-evaluation-v1.md): RAG 전후 비교, 필수 Metric, Schema Set 1.5 `Candidate · Review Required` — 1.4의 Grounding/Safety projection 23개 member를 그대로 보존하고 #159용 `answer-human-judgment`, `answer-human-judgment-approval`, `answer-comparison-set-manifest` 3개 artifact를 추가한 26-member DEV 계약; Release 차단 기준 유지
- [RAG Answer Quality Metric·Variant 계약 v1 (#159)](./targets/post-mvp-1/rag-answer-quality-metrics-v1.md): PR #475 책임 리뷰 승인으로 확정된 Approved Target. `REQUIRED_CLAIM_RECALL`·`COMPLETENESS` 순수 DEV kernel과 manifest routing을 구현했고, Schema Set 1.5 Candidate에서 human judgment·approval·3-pair comparison-set 입력 schema를 추가했다. `ANSWER_CORRECTNESS`·`RELEVANCE` DEV kernel과 `ANSWER_CORRECTNESS` zero-claim bootstrap replicate semantics 및 report projection capability를 구현했으며, 실제 authoritative human judgment runtime wiring, 3-pair comparison builder·Variant 실행은 후속 구현이고 HOLDOUT·Release 승인은 별도 게이트.
- [Answer Runtime Authority Binding Manifest 계약 v1 (#159)](./targets/post-mvp-1/answer-runtime-binding-manifest-v1.md): PR #833 책임 리뷰 승인으로 확정된 Approved Target · Partially implemented. Issue #159 Answer 3-pair comparison(`ANS-BASE--ANS-RAG`, `ANS-RAG--ANS-FINAL`, `ANS-BASE--ANS-FINAL`) 실행 시 요구되는 15개 authority 중 10개 Canonical Recipes(7 supplemental + 3 delta)와 1개 명시적 미해결 바인딩(`RETRIEVED_EVIDENCE`)의 규격을 정의하고, 7개 Supplemental Controls에 대해 Recipe 정의·기존 소스 객체·3-Variant 실행 캐리어 상태를 엄격히 분리(`source exists != carrier ready`)하며, 비적용 축의 명시적 `NOT_APPLIED` typed state 정의로 `ANS-BASE -> ANS-RAG`가 4개 finalization authority와 구조적으로 독립됨을 확립한다. Phase B Python 부분 구현으로 manifest DTO/schema, 승인된 10 recipe helpers, self-hash/run/variant delta state validation, #808 typed seam projection·fail-closed tests를 완료했다. Phase C-1/C-2를 통해 9개 바인딩의 정본 권위 카디널리티(`RUN_LEVEL_CONFIG` 1개, `ADAPTER_LEVEL_ACTUAL` 4개, `CASE_LEVEL_ACTUAL` 3개, `UPSTREAM_AUTHORITY` 1개)와 케이스 집계 불변식($\forall c, \, \text{value}(c) == \text{value}_0$), `ALL_CASES_COMPLETED_PRE_SEAL` 생성 시점, #808과 정합하는 Run 번들 외부 Experiment-level companion 저장 좌표(`<experiment_dir>/binding-manifests/<variant_id>.json`), 및 #162 Evaluation Guard Evidence 정합(`PENDING_162_AUTHORITY_FREEZE`)을 제안(Proposed / Review Required)했다. 7개 supplemental authoritative runtime carrier/extractor, `RETRIEVED_EVIDENCE` authority/carrier, 4개 finalization authority, actual 3-variant pair execution은 미구현이며 계속 `NOT READY`(fail-closed)다. `run_id`(`CanonicalUuid` 정렬)를 포함한 run-scoped manifest artifact integrity hash(`manifest_sha256`) 규정 및 cross-run 동등성의 개별 바인딩 해시 위임, `rag-eval.run` Schema Set 1.5 불변 유지 및 `answer-comparison-set-manifest`와의 책임 분리를 유지한다.
...
- [Guide Runtime Release/Fallback Projection v1 (#180)](./targets/post-mvp-1/guide-runtime-release-v1.md): #890/#893 internal release result를 versioned `rag_runtime` shared output carrier로 사영하는 순수 경계 계약. Adapter는 Card↔validated selection target anchor만 확인하고 #794/#869/#882 exact-binding semantics를 재구현하지 않는다. Backend public DTO·Evaluation Release Gate·Worker·LangGraph는 포함하지 않음
- [Guide Medication Guidance Retrieval Binding v1 (#180)](./targets/post-mvp-1/guide-medication-guidance-retrieval-binding-v1.md): Guide `retrieve_medication_guidance` authoritative input matrix. B2 producer/verifier and composition-root injection plus B5 exact terminal replay readback are READY; B1 still blocks the canonical callable.
- [Guide Medication Retrieval Query Authority v1 (#180 B2)](./targets/post-mvp-1/guide-medication-retrieval-query-authority-v1.md): verified medication의 pinned name/nullable strength snapshot을 one-medication query text로 exact projection하고 approved B2 fingerprint authority로 전달하는 Target 계약.
- [Guide Retrieval Query Fingerprint Authority v1 (#180 B2)](./targets/post-mvp-1/guide-retrieval-query-fingerprint-authority-v1.md): versioned HMAC producer/verifier, typed Backend key injection, and secret-free verifier artifact identity authority.
- [Sync Chat CLOSED_DEMO Retrieval Delta v1 (#180)](./targets/post-mvp-1/sync-chat-closed-demo-rag-v1.md): sealed 17p read-only composition exception, Chat-only query-binding authority, evidence payload/v7 prompt and exact Backend→Worker import boundary. Current v6/public Track F는 변경하지 않음.
- [Guide REQUEST Authority Coordinate Lookup v1 (#180 B3)](./targets/post-mvp-1/guide-request-authority-coordinate-lookup-v1.md): 원 요청 identity와 selected Source/Snapshot/Member 좌표를 기존 historical REQUEST Source/Member Decision artifact ref에 equality-only로 결속하는 read-only lookup 계약. latest/current fallback, Decision 재판정, migration은 포함하지 않는다.
- [Guide Retrieval Outcome Binding v1 (#180 B4)](./targets/post-mvp-1/guide-retrieval-outcome-binding-v1.md): first-run `HybridRetrieveOutcome`에서 #697이 소비하는 status/receipt/gate 표면만 pure fail-closed로 투영한다. terminal replay historical payload recovery(B5)는 포함하지 않는다.
