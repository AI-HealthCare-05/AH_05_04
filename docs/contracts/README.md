...
- [Guide Generation Canonical Gap Closure v1 (#180)](./targets/post-mvp-1/guide-generation-canonical-gap-closure-v1.md): Guide Graph의 selection·safety filter·conflict gate·composition 네 node와 현 production 의미를 대조한다. #774 projection, Card post-generation validation, fallback vocabulary를 허위 canonical mapping으로 사용하지 않고, provider invocation만 composition-only callable로 최소 추출한다. 나머지 세 node는 승인된 policy/carrier 부재로 contract-alignment blocker다.
