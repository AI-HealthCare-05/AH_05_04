...
- [Answer Runtime Authority Binding Manifest 계약 v1 (#159)](./targets/post-mvp-1/answer-runtime-binding-manifest-v1.md): PR #833 책임 리뷰 승인으로 확정된 Approved Target. Issue #159 Answer 3-pair comparison(`ANS-BASE--ANS-RAG`, `ANS-RAG--ANS-FINAL`, `ANS-BASE--ANS-FINAL`) 실행 시 요구되는 15개 authority 중 10개 Canonical Recipes(7 supplemental + 3 delta)와 1개 명시적 미해결 바인딩(`RETRIEVED_EVIDENCE`)의 규격을 정의하고, 7개 Supplemental Controls에 대해 Recipe 정의·기존 소스 객체·3-Variant 실행 캐리어 상태를 엄격히 분리(`source exists != carrier ready`)하며, 비적용 축의 명시적 `NOT_APPLIED` typed state 정의로 `ANS-BASE -> ANS-RAG`가 4개 finalization authority와 구조적으로 독립됨을 확립하되 `RETRIEVED_EVIDENCE` 미해결 및 7개 supplemental controls 실행 캐리어/추출기 미구현으로 인해 현재는 `NOT READY`(fail-closed) 상태임을 명시하며, 미완성 4개 런타임 authority(Final Validator, Citation Gate, Safety Gate, Release Gate)는 가상값 없이 `BLOCKED_BY_UPSTREAM_AUTHORITY`(`null`)로 격리하여 PR #808 typed seam(`AnswerComparisonSupplementalControls`, `AnswerComparisonDeltaBindings`)에서 자동 fail-closed 처리하는 독립 manifest 계약. `run_id`(`CanonicalUuid` 정렬)를 포함한 run-scoped manifest artifact integrity hash(`manifest_sha256`) 규정 및 cross-run 동등성의 개별 바인딩 해시 위임, `rag-eval.run` Schema Set 1.5 불변 유지 및 `answer-comparison-set-manifest`와의 책임 분리. Phase B 계약 승인 완료로 manifest DTO/schema, 10 canonical recipe helpers, manifest validation, #808 typed seam projection 및 fail-closed 처리의 Python 구현 착수를 허용한다.
