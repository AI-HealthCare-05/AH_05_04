...
- [Guide Medication Guidance Retrieval Binding v1 (#180)](./targets/post-mvp-1/guide-medication-guidance-retrieval-binding-v1.md): Guide `retrieve_medication_guidance` production binding의 authoritative input freeze. B5 exact terminal replay persistence/readback은 구현됐지만 B1/B2와 최종 callable은 아직 READY가 아님.
- [Guide Medication Retrieval Query Authority v1 (#180 B2)](./targets/post-mvp-1/guide-medication-retrieval-query-authority-v1.md): verified medication의 pinned name/nullable strength snapshot을 one-medication query text로 exact projection하는 Target 계약. B2-2의 exact algorithm authority gap이 fail-closed 상태다.
- [Guide Retrieval Query Fingerprint Authority v1 (#180 B2-2)](./targets/post-mvp-1/guide-retrieval-query-fingerprint-authority-v1.md): Approved `PD-315-20260908`의 generic versioned HMAC direction과 Current/Target runtime을 대조한 B2-2 authority audit. exact algorithm/key-version/key-owner/Worker injection/verifier artifact authority가 없어 production implementation은 추가하지 않는다.
