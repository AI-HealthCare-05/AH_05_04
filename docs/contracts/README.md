...
- [Check-in과 Barrier 계약 v1](./checkin-v1.md): 3개 Check-in 결과와 Barrier 미응답 표현
...
- Barrier 미응답은 `response_status=DECLINED`, `barrier_code=null`로 표현한다.
...
- timed occurrence는 정확한 처방 시각 또는 사용자 확인 일정에서만 만들고, Check-in deadline은 `max(Asia/Seoul 기준 예정일 다음 날 00:00, scheduled_at + 4시간)`을 UTC instant로 snapshot한다.
