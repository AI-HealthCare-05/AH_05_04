...
- [Safety Result 복합 STALE 우선순위 계약 제안 v1 (`PD-173`)](./proposed/post-mvp-1/safety-result-compound-stale-priority-v1.md): 처방 버전·식별 스냅샷·런타임 번들 복합 STALE 발생 시 단일 fallback_code 사영 우선순위(`PRESCRIPTION_STALE` > `IDENTIFICATION_STALE` > `RUNTIME_RELEASE_STALE`) 정의. 문서 상태 Approved — PR #382 최신 HEAD에서 책임 리뷰 승인 완료(2026-09-09), 승인 evidence는 [`docs/validation/rag/issue-173/decision-approval-evidence.json`](../validation/rag/issue-173/decision-approval-evidence.json). 승인은 어휘·우선순위 규칙의 확정을 의미하며 `targets/` 승격이나 Production 활성화를 의미하지 않는다.
