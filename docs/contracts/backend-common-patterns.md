...
현재 복약 가이드·OCR·챗봇 MVP 구현은 외부 호출 실패 시점에 즉시 `commit()`해서 실패 상태를 보존합니다. 이 `commit()`은 실패 상태 필드만 선택적으로 저장하는 것이 아니라 **같은 DB 세션에 그 시점까지 pending 상태로 남아있던 다른 모든 변경사항까지 함께 커밋**합니다.

이 방식은 현재 요청 하나가 단일 DB 세션으로 동기 처리되는 MVP 구현에서만 쓰는 예외적인 패턴이며, 새 기능에 그대로 적용해도 되는 일반 권장 패턴이 아닙니다. 같은 세션 안에서 savepoint나 nested transaction을 나누어도 바깥 세션의 `commit()` 영향이 완전히 격리되지는 않습니다.
...
구체적인 `flush()`/`commit()` 동작 차이, 코드 예시와 도메인별 메서드 이름(`mark_failed`, `commit_failed_message_pair` 등)은 Backend 내부 구현 가이드로 옮깁니다.
