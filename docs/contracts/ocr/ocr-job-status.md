...
- 문서에 연결된 OCR 작업을 `job_id`로 명시적으로 식별해 사용자가 검수한 작업과 확정 대상 작업을 일치시키는 검증은 [처방 확정 Backend 계약](../prescriptions/prescription-confirmation.md)의 Post-MVP 이관 항목을 따릅니다.

## 검증과 변경 규칙

구현 계약은 `app/tests/ocr`에서 검증합니다.
