...
Post-MVP-1의 Job·결과와 Track B·C, Candidate·Identification 직접 API는 parent resource를 따라 동일한 `user_id` 소유권을 검사합니다. 인증 사용자가 직접 소유하지 않은 식별자는 존재 여부를 숨기기 위해 `404`로 응답합니다. 이 규칙은 Approved target이며 현재 endpoint별 구현 여부는 코드·OpenAPI·테스트로 따로 확인해야 합니다. 보호자·patient profile·대리 요청은 후속 계약이며, 내부 운영자는 감사되는 별도 support role 없이 의료 결과를 조회할 수 없습니다.
