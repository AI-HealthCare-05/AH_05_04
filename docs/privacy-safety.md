...
- Local에서는 비식별 합성 데이터 검증을 위해 flag의 `true`와 `false`를 허용합니다. Staging은 활성화가 금지됩니다. Issue #838 Accepted Decision과 외부 승인 게이트 `EXT-PRIV-003` 승인에 따라 Production은 기본값 `false`를 유지하며 명시적 opt-in(`true`)만 허용합니다. 이 승인은 실제 Production 활성화를 자동 수행하지 않습니다.
...
실사용 개방 전 고지·처리 근거, 검토자 권한·감사, 만료 삭제 스케줄·백업 파기를 Privacy·Security 담당자가 확인해야 한다. 제품/운영 기준 확인은 이 선행 조건을 문서화하는 것이며 실제 수집 승인으로 해석하지 않는다.
