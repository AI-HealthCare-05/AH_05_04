...
1. 기존 migration owner로 단일 head `3980718293a4`까지 적용하고 `scripts/ci/verify_database_head.py`로 사용자 Trigger/RLS/제거 함수 0개를 확인한다. 이미 적용한 migration 파일은 변경하지 않는다.
...
관리 기능이 필요 없으면 관리 프로필을 실행하지 않는다. 서비스는 자동 배포·재시작 의존성에 추가하지 않았다. 새 migration의 downgrade는 먼저 감사 테이블을 잠근다. 진행 중 감사 저장이 끝난 후 기록 존재 여부를 다시 확인하고, Audit가 존재하면 거부한다. 운영 rollback은 audit 보존 계획과 담당 검토를 전제로 한다.
...
`#398` 먼저 병합 → `#291`의 입력·정보 계약에 맞춰 `#372` 진행. #372에는 이미 `c263698`(Trigger 제거), `7e1c113`(Python 검증), `6f1acc9`(develop 통합)이 반영되어 있지만, #398 병합 후 migration 부모와 신규 Catalog Writer 정책을 다시 연결해야 한다. 이 문서가 #372 병합 준비 완료를 뜻하지 않는다. #404는 병합 시점에 별도로 반영한다.
