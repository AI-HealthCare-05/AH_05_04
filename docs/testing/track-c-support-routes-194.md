...
#629가 `c0e8798a`로 develop에 병합되어 최신 화면·접근성 수정과 #639의 사유별 기능을 합쳤다.
공통 원본 #629 커밋을 기준으로 양쪽 변경을 대조해 공통 shell·최신 문구·대상 약 저장 검사를
보존하고, 상황 선택·자료 조회·읽기 전용 알림 확인·완료/취소 회귀를 유지했다.
#629·#635 선행 병합은 완료됐으며, 통합 diff의 최종 책임 리뷰와 CI는 별도 확인한다.

통합 후 Frontend 731개·Track C 모바일 브라우저 5개·TypeScript·oxlint·production build·diff 검사가 통과했다. 실제 API 왕복은 이번 통합 라운드에서 재실행하지 않았으며 앞선 검증과 구분한다.
