...
| RAG Candidate Index | `rag_candidate_index_version`, `rag_candidate_index_member` | #168(RAG-07B) RAG-07A(#167) build 결과의 영속·멱등 build transaction. `status`는 항상 `BUILDING`이며 READY/RETIRED 전환은 RAG-17(#180) 후속 범위 |
...
막는다. `READY`/`RETIRED` 전환과 환경 pointer 연결은 RAG-17(#180) 후속 범위이며, partial/failed
