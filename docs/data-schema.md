...
| RAG 실행·안전 결과 | retrieval run·signal·hit, result·claim·citation·safety 계열, `rag_citation` | `rag_citation`은 승인된 Evidence와 동일 Source Snapshot에 묶인 claim-Evidence 공개 가능성 최소 저장만 제공. 이번 최소 DB 기반의 Evidence 상태는 `DRAFT`/`APPROVED`, Citation 공개 상태는 `NOT_PUBLIC`/`PUBLIC`만 사용하며, STALE·retire·revoke lifecycle은 후속 전환 설계에서 추가한다. 실제 Retrieval, Provider 호출, 답변 생성, Safety/Fallback 문구 생성, 화면 표시는 후속 |
