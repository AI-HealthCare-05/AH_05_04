...
| 의료 지식 | `knowledge_document`, `knowledge_chunk`, `rag_knowledge_index`, `rag_knowledge_index_member`, `retrieval_run`, `retrieval_signal`, `retrieval_hit` | #178 Knowledge Evidence Index 저장 기반 및 PostgreSQL Evidence Search + 결정적 RRF + Retrieval Run persistence 구현 브랜치. revision `178b1c2d3e4f`에서 lexical GIN 인덱스 추가, `178c2d3e4f50`에서 retrieval run/signal/hit 테이블 추가. Reranker·actual evaluation·공개 활성화는 후속 범위 |
...
| RAG Runtime Bundle Citation Pin | `rag_runtime_bundle_citation_approval` | #853 Runtime Bundle canonical content가 소비할 exact historical #807 `PATIENT_CITATION` approval을 전용 child row로 pin한다. migration `853a1b2c3d4e`, manifest projection v2, Worker exact-pair read를 사용하며 NEW C 판단은 포함하지 않는다. |
...
이메일 발송은 `email_verification_token`이나 `password_reset_token` 테이블에 원문 token을 저장하지 않고 `EmailSender` adapter 경계에서만 원문을 사용합니다. 기본값 `EMAIL_PROVIDER=noop`은 발송하지 않으며, 이번 범위에서 SMTP adapter는 local 검증용으로만 사용할 수 있습니다. Production/Staging SMTP 활성화와 실제 Provider 계정·비용 정책은 후속 보안·배포 설정 PR에서 확정합니다.
...
### #178 Knowledge Evidence Index 선행 저장 기반

Revision `178a1b2c3d4e`는 기존 Knowledge 골격을 폐기하지 않고 다음 저장 경계를 추가한다.

- `rag_source_snapshot_member`: 하나의 unsealed `PENDING` Snapshot 안에서 Endpoint/Operation 또는 captured
  Artifact origin 하나만 선택하고 locator와 content SHA-256을 결속한다. 이후 `CURRENT` 전이가 member set을
  고정하며 index adapter는 CURRENT Snapshot만 소비한다.
- `knowledge_document.record_contract_version`: 기존 row는 `LEGACY_V1`로 보존하며 새
  `KNOWLEDGE_EVIDENCE_V1` row만 Snapshot member·external document ID·document hash·canonicalization
  version을 요구한다.
- `knowledge_chunk`: legacy external vector 필드는 nullable로 유지하고 production Chunk에는 content hash와
  normalization version을 요구한다.
- `rag_knowledge_index`: index code/version, model ref/version, dimension, `COSINE`, corpus·embedding·configuration
  hash와 member count를 보존하는 완성 artifact다.
- `rag_knowledge_index_member`: stable Chunk 좌표, Source Snapshot 결속, PostgreSQL `vector`, 개별 embedding
  SHA-256과 결정적 member order를 보존한다.

Python adapter는 Source→Endpoint→Operation→Snapshot→member→Document→Chunk를 잠가 exact-match한 뒤 한
transaction에서 전체 index를 기록하고 persisted row로 receipt를 재계산한다. 동일 code/version은 세 hash와
...
Revision `178c2d3e4f50`은 #178 Knowledge Evidence Search의 결과를 재현 가능하게 영속화하기 위해 세 테이블을 추가합니다:
- `retrieval_run`: `(job_id, node_id)` unique, variant(`RET-L|RET-D|RET-H`), status(`RUNNING|COMPLETED|FAILED`), manifest/config/filter 해시, limits, 시작/완료 시각을 보존하는 실행 헤더.
- `retrieval_signal`: Exact, Trigram, FTS, Lexical-fused, Dense의 raw 신호와 순위 및 canonical score를 보존.
- `retrieval_hit`: candidate당 1행으로 RRF 순위, 분수 receipt, final_rank, selected(`selected=true`는 pre-gate Top 5 중 Gate 통과 항목)를 보존.
Downgrade는 세 테이블을 안전하게 역순으로 drop합니다.
...
- `rag_candidate_index_version`: `index_code`+`index_version` unique, `content_hash` unique(멱등성
  키), `catalog_set_id`로 `rag_catalog_set.id`를 FK RESTRICT 참조. #168 build transaction은 새 row를
  `BUILDING`으로 만들고, #583 lifecycle transaction은 완성된 `BUILDING`만 `READY` 또는 `FAILED`로
  닫는다. `(index_code) WHERE status = 'BUILDING'` partial unique index로 같은 index에 대한 동시
  build를 막고, `(index_code) WHERE status = 'READY'` partial unique index로 RAG-08/RAG-09 read path의
  active READY를 code당 최대 1개로 제한한다. `build_mode = 'LEXICAL_ONLY'`와 embedding 관련 컬럼 전부
  NULL, `HYBRID`와 전부 NOT NULL을 CHECK 제약으로 강제한다.
...
수행하며, 전달된 member 행으로부터 각 `member_content_hash`, `member_set_hash`, count를 재계산해
claim된 값과 대조함으로써 위조를 막는다. #583 lifecycle은 같은 transaction 안에서 기존 READY를
`RETIRED`로 보존한 뒤 새 READY를 승격하고, member row 수와 count/hash 정합성이 맞는
완성된 build만 READY가 되도록 확인한다. 환경 pointer/Runtime Bundle FK 연결과 최종 release gate는
RAG-17(#181) 후속 범위다.
...
Runtime Bundle 최소 DB 기반은 `rag_runtime_execution_manifest`로 실행 manifest와 Worker/model/prompt/parser/resolver/guard reference를 고정하고, `rag_runtime_release_bundle`로 bundle manifest hash와 Candidate/Knowledge index ref/hash를 보관합니다. `rag_runtime_bundle_source`는 bundle이 선택한 `rag_source_snapshot.id`를 직접 FK로 기록합니다. `rag_release_evaluation_approval`은 `bundle_id + bundle_manifest_hash`와 `eval_run.id + decision_status`를 FK로 묶어 승인 기록이 정확한 Bundle manifest와 실제 PASS Run에서 벗어나지 않도록 하며, `rag_runtime_environment`는 active bundle id와 manifest hash를 함께 저장합니다. `rag_runtime_environment_transition`은 activation·rollback·resume·suspend 시 이전/이후 bundle pointer와 Guard Decision ref를 append-only 이력으로 보존합니다. 활성 여부의 기준 원본은 `rag_runtime_environment.active_bundle_id + active_bundle_manifest_hash` 포인터이며, `rag_runtime_release_bundle.bundle_status`에는 `ACTIVE` 값을 두지 않습니다. `rag_runtime_environment.environment_code`와 `rag_runtime_release_bundle.environment_code`는 canonical Runtime Environment 어휘(`LOCAL`, `TEST`, `CLOSED_DEMO`, `PRODUCTION`)로 제한되며 exact case-sensitive 문자열로 취급하고 CHECK 제약조건(`chk_rag_runtime_environment_code`, `chk_rag_runtime_bundle_environment_code`)으로 강제합니다(PD-799-20260918, Issue #810). #398은 최소 Environment 전이 transaction과 이력 원자성을 구현합니다. 전체 drain, mixed Worker rollback, Candidate Index 연결과 Production 공개 승인은 후속 범위입니다.

#853의 `rag_runtime_bundle_citation_approval`은 exact `(bundle_id, bundle_manifest_hash)`의
canonical child로서 #807 `PATIENT_CITATION` approval ID와 Snapshot/Source/version/environment/purpose
facts를 보존합니다. `(bundle_id, source_snapshot_id)` UNIQUE, parent exact-pair FK, Snapshot 및
Source Use Approval `RESTRICT` FK, `purpose='PATIENT_CITATION'` CHECK를 사용합니다. 이 row set은
manifest projection v2의 hash input이며 Runtime writer에는 `SELECT, INSERT`, Worker에는 read-only
조회 경계만 제공합니다. semantic UPDATE/DELETE/TRUNCATE API는 없습니다.
...
OCR Candidate Index와 Knowledge Evidence Index는 별도 version, hash, table, vector port와 테스트 경계를
가진다. pgvector는 각 경계에서 독립적으로 사용하며 Candidate vector를 의료 근거로 재사용하지 않는다.
HIRA 적용약가 데이터는 공식 제품 식별 입력·정답 원장·상호작용 근거로 사용하지 않습니다.
...
#178 migration `178a1b2c3d4e`는 Knowledge Index build가 parent chain을 `SELECT FOR UPDATE`로 검증할 수 있도록
관련 Source·Knowledge·Index row에 `knowledge_index_lock_marker INTEGER NOT NULL DEFAULT 0 CHECK (... = 0)`를
둔다. Snapshot은 기존 `management_lock_marker`를 공유한다. 전용 Builder는 이 고정 컬럼만 UPDATE할 수 있고
Source·Knowledge 업무 필드는 수정할 수 없다. Builder는 Knowledge document/chunk와 Index/member에
...
## #633 피드백 저장 — Local 구현 병합, Proposed
