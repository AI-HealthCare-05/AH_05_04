...
| 챗봇 최근 대화 활성화 | 실제 `CHAT_HISTORY_CONTEXT_ENABLED`: ____ (기본값 `false`, Staging은 반드시 `false`(true는 Config validation 거부), Production은 외부 승인 전 반드시 `false` 유지; Issue #838 Proposed Decision 검토 중) |
...
- Staging 및 승인 전 Production의 `CHAT_HISTORY_CONTEXT_ENABLED=false`를 확인합니다. Staging에서는 `true` 설정 시 Config validation에서 거부됩니다. 배포 스크립트(`scripts/deployment.sh`)는 외부 LLM 전송 승인(`EXT-PRIV-003` / Issue #838) 전까지 `CHAT_HISTORY_CONTEXT_ENABLED=true` 배포를 차단하며, `history: []`만 허용합니다. 향후 정식 승인 후 Production 명시적 opt-in 시에는 FastAPI 컨테이너에만 전달되며, 롤백 시 `false` 재설정 및 FastAPI 컨테이너 재생성이 요구됩니다.
...
| OpenAI 챗봇 | 현재 질문과 확정 약물 필드. 최근 완료 대화 최대 3쌍은 별도 승인(`EXT-PRIV-003` / Issue #838) 후에만 허용하며 현재 Staging(Config 거부) 및 승인 전 Production에서는 `history: []`: ____ | `store=False` 포함 실제 정책 확인: ____ | ____ |
