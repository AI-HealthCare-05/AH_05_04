...
`ACTION_PLAN_FOLLOWUP_REVISION_CONFLICT`다. 세부 필드·순서는 [후속 계약](../../contracts/current/track-c-followup-api-194.md)을 따른다.
...
김지혜의 상태 정렬 요청에 따라 PR #631의 구현·OpenAPI/DTO·테스트·증빙과 함께 계약을 `current/`에 반영한다.
최종 책임 리뷰 승인·병합은 대기 중이다. 병합 전 develop의 Current runtime 완료나 외부 공개 승인으로 해석하지 않는다.
