...
1. **PD-315 상태와 제안(Proposed) 경계**:
   - 상위 결정 `PD-315-20260908`은 PR #361 최종 HEAD에서 지정 책임 리뷰어 승인을 받아 `Approved`다([승인 provenance](./2026-09-08-production-evidence-retrieval-contract-divergence.md#승인-provenance)). 다만 Decision 승인은 구현·Current 승격·Runtime 활성화를 의미하지 않으므로, 본 결정에서 정의하는 `operation_code` nullable 규칙의 채택은 여전히 **제안(Proposed)** 단계이며 순수 계약 계층에만 적용된다.
...
1. **`docs/contracts/current/` 승격 증빙**: `PD-315-20260908`은 `Approved`이므로 더 이상 차단 사유가 아니다. 그러나 Decision 승인은 구현 증빙이 아니므로, 구현·migration·OpenAPI/DTO·자동 테스트·실행 증빙을 갖춘 별도 PR 전까지 본 계약은 `Proposed`로 유지한다.
