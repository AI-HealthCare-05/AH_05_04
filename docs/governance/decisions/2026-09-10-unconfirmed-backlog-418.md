...
- 구현 상태: repository·service·DTO·미등록 router 및 합성 테스트 작성. 공개 API 아님.
...
API route·DTO·오류는 공유 계약 변경이므로 `AGENTS.md`의 사전 조율 규칙에 따라 승인 전에는 `apis/v1/__init__.py`에 등록하지 않는다. 후보 router는 테스트 전용 앱에서 검증한다. 책임 리뷰어들이 URL·필드·순서·cursor·오류·Frontend 재조회 흐름을 승인한 뒤 등록과 실제 #202 PUT 연동 검증을 완료해야 한다. 이 Draft 구현과 테스트만으로 승인, Current 승격, #418 종료를 주장하지 않는다.

## Pagination 한계

전체 페이지는 시점 고정 snapshot이 아니다. 후속 GET은 그 시점의 UNCONFIRMED만 반환하며, cursor 앞에 나중에 생성된 과거 기록은 처음부터 재조회해야 보인다. 없는/삭제된 cursor는 최초 페이지부터 재시작한다. 처방과 계정의 기존 삭제 정책을 변경하지 않는다.
