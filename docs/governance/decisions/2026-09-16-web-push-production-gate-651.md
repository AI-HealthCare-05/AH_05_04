...
3. 이 플래그를 끄면 **새 설정을 읽은 API/scheduler 인스턴스**의 신규 config/등록 호출과
   전송 batch가 거부된다. 이미 이전 값을 메모리에 올린 API/scheduler 프로세스는 `@lru_cache`로
   인해 재시작 전까지 이전 값을 계속 사용한다. 따라서 OFF 절차의 완료 시점은 API/scheduler
   전체 인스턴스 재시작이 끝난 뒤이며, 코드 롤백이나 재배포가 필요하다는 뜻은 아니다.
