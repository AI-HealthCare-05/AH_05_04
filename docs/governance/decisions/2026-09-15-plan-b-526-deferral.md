...
김지혜가 공유한 9월 15일 10:36 정현우 답변은 기존 승인 Source/Snapshot만으로 제품 Identity와 MFDS 환자용 복약정보를 연결할 수 있는지 짧게 확인하되, 부분 Catalog 구성 또는 자연키·locator·추가 승인 문제 해결이 필요하면 이번 범위에서 제외하도록 제안했다.

같은 날 11:30 정현우는 공유된 Receipt를 근거로 환자용 복약정보 Source의 현재 Runtime 연결이 불가능하고 별도 자연키·locator 해결이 필요한 상태임을 확인했다. 일정과 중단 조건을 고려해 이번 시연에서는 제품별 근거 RAG를 위한 추가 Catalog 승인 연결을 진행하지 않고, 확정 처방 안내·Safety Intake·긴급/응급·범위 제한·근거 부족 시 안전 종료를 우선 구현하는 방향에 동의했다.

예외는 추가 조사나 계약 변경 없이 즉시 사용할 수 있는 승인 Snapshot·자연키·locator 증빙이 이미 준비돼 있는 경우다. 해당 자료가 없다면 제품별 근거 RAG는 후속 범위로 둔다. 이는 이번 시연 범위에 대한 확인이며, 새로운 Source 승인이나 Runtime 활성화 승인을 의미하지 않는다.

10:36 및 11:30 답변 원문은 김지혜가 공유한 대화 캡처로 확인했다. 저장소에서 추적 가능한 메시지 링크는 아직 확보하지 않았다.
...
1. 추가 조사나 계약 변경 없이 즉시 사용할 수 있고, 제품 Identity·필수 데이터가 완전한 기존 승인 Source/Snapshot과 동일 제품의 MFDS 환자용 복약정보 연결 증빙이 이미 준비돼 있어 제시된다.
