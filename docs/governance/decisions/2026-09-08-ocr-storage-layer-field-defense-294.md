...
- `PRESCRIBED_DATE`(index 0): #352 당시 규칙 경로는 날짜 미인식 시 row를 만들지 않아
  저장 계층이 보충했다. #353/#309 통합에서는 규칙 경로도 날짜 누락·제외·상충 시
  빈 검수 필드를 생성한다. 저장 계층은 이 기존 행을 보존하고 row 자체가 누락된
  입력만 방어적으로 보충한다. 따라서 통합 이후 저장 계층이 유일한 방어선은 아니다.
