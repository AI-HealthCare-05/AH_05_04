...
문서 승인, 구현 완료, 자동 테스트 통과, 외부 승인과 사용자 공개는 서로 다른 상태다. `docs/contracts/`의 도메인 하위 디렉터리에 있는 `*-v1.md` 목표 문서는 관련 영역의 지정 리뷰어가 승인한 뒤 구현 시 따라야 할 normative target이지만, 구현 PR이 병합되기 전에는 현재 API·DB 동작이 아니다. 제품·기획 문서는 의도와 우선순위를 설명하며 runtime 계약을 대체하지 않는다.
