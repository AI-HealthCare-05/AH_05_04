...
- **SELF profile 소유권 이관:** #117 구현 PR에서 사용자당 `SELF` profile 1개를 보장하는 제약, 기존 의료 데이터의 `profile_id` backfill, FK·index 생성과 read/write cutover 순서, rollback, endpoint별 권한 테스트를 함께 반영한다. #117 병합 이후 새 Backend 리소스의 소유권 기준은 SELF `profile_id` 또는 부모 chain의 `profile_id`를 따른다. 보호자·멀티 프로필·위임 권한은 후속 Decision이 승인될 때까지 구현하지 않는다.
