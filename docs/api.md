...
`purpose`는 `OCR`, `GUIDE`, `CHAT`, `NOTIFICATION`만 허용합니다. 요청 body는 `status`(`GRANTED` 또는 `WITHDRAWN`)와 `policy_version`(1~100자)을 받습니다. 응답은 목적별 `status`, `policy_version`, `is_granted`, `granted_at`, `withdrawn_at`, `updated_at`을 반환합니다. row가 없는 목적은 `status=null`, `policy_version=null`, `is_granted=false`로 반환해 미동의와 명시 철회를 구분합니다.

이 API는 PD-207의 목적별 최신 동의 상태 저장·조회·변경 경로입니다. OCR/Guide/Chat 접수 Gate, Worker 실행 직전 재검사, `CONSENT_REQUIRED`, OCR `CONSENT_WITHDRAWN`, 동의 철회 `STALE + BLOCKED` 종료 전이는 후속 구현 범위입니다.
