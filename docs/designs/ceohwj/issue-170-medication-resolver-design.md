...
| `#171` Candidate Search·Identification | Backend model/repository/service와 Deferred Constraint가 존재한다. `record_candidate_search(...)`, `finalize_candidate_search(...)`, 확인·거절·preflight 경계가 있으나 공개 3개 route는 항상 `503 SERVICE_UNAVAILABLE`로 fail-closed한다. | Candidate 저장 slice는 구현됐지만 #169 ownership chain, #166/#168 Source·Index와 #170 Resolver가 연결되지 않았다. 공유 Target 전체는 Not implemented이며 공개 사용 불가다. | OCR input Receipt `sha256:e134ad8ff700050456d5d77976336b61a795207cafbe12f385cb7c9bba2c92fe`와 Source gate `sha256:d687e75ebfbb3bc10b9887280e5c994bcc7bc0481722c660c6ce2cda8c3d402a`; 전용 #171 completion Receipt 없음, `READY=false`. |
...
| --- | --- | --- | --- | --- |
| `G0` 착수·dependency gate | `BLOCKED` | #166 Source/Catalog completion Receipt, #168 active Candidate Index Receipt, #169 활성 Medication Snapshot Receipt가 모두 연결된 뒤 Resolver 통합을 시작한다. 그 전에는 순수 합성 contract test 설계만 허용한다. | 현재 `medication`·nullable index ID·합성 Source Receipt로 통합을 시작하지 않는다. 서로 다른 목표 객체를 Current로 승격하기 때문이다. | 정현우 + 김지혜·송은영·권가빈 / 세 선행 Receipt와 새 dependency snapshot / Task 1 입력 adapter·합성 fixture 착수 |
...
기본 불변식 권고안은 `0 <= eligible_count <= deduped_count <= raw_count`,
`persisted_count = deduped_count`, `displayed_count in {0, 1}`이다. G2의 invalid raw hit는 count 전에
port failure로 닫는다. 이 count 정의 자체가 G6 승인 전에는 구현 계약이 아니다.
