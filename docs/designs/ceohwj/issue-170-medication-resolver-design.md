...
| `G5` Single Candidate outcome | `TBC (BLOCKED BY G0)` | eligible 공식 Product Identity가 정확히 1개일 때만 Resolver는 `SINGLE_CANDIDATE`를 판정하고, #171 Finalizer가 이를 lifecycle `READY`와 공개 후보 1개로 투영한다. 2개 이상은 `AMBIGUOUS`; 0개는 진단 분기에 따라 `NO_CANDIDATE`, `INGREDIENT_ONLY`, `INVALID_INPUT`이다. | top-1 score 자동 선택, threshold 미만 후보 표시, `SINGLE_CANDIDATE`를 새 공개 status로 추가, `AMBIGUOUS`에서 Top-K 공개는 채택하지 않는다. | 권가빈 + 정현우 / Resolver Policy 승인과 Single Candidate Gate evaluation Receipt / Task 5 outcome classifier |
