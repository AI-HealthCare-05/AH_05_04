...
│   ├── manifests/rag-natural-language-retrieval-dev-v1.authoring-identities.json
...
`SYNTHETIC_REPLAY_DEV`와 혼용하지 않는다. 이 label은 allowlist된 actual Adapter가 registry에서 resolve된
Run에만 사용하며, 실행 성공을 뜻하지 않으므로 별도 execution status를 항상 함께 표시한다. unknown 또는
unresolved Adapter를 `ACTUAL_RETRIEVAL_DEV`로 표시하지 않는다.
...
Index build receipt와 study split receipt는 기존 `ProtectedArtifactReceipt`로 대신하지 않는다. 단계 C/D 전에
별도 Decision과 Evaluation Contract Freeze에서 다음 machine contract를 승인한다.

- `rag-eval.index-build-receipt@1.0.0`
- `rag-eval.study-split-receipt@1.0.0`
- `rag-eval.authoring-identity-manifest@1.0.0`

계약 문서는 `docs/contracts/targets/post-mvp-1/`와 계약 index에 등록하고, 새 Schema Set version에서 schema
...
계약과 Schema Set 승인 전에는 임의 JSON Receipt를 생성하거나 실제 Run 입력으로 사용하지 않는다.

현재 CLI는 `run-dev`와 repository-root DEV config만 지원한다. protected root loader,
`run-protected-holdout`, authorization receipt, 결과 저장 root와 접근 audit는 `#157` 잔여 작업 또는 별도
보호 실행 Issue의 선행 산출물로 둔다. 해당 경계가 구현·승인되지 않으면 단계 D를 실행하지 않고 #273을
Open으로 유지한다.
...
- Dataset Freeze가 승인되면 별도 전이로 `dataset.status=FROZEN`과 `frozen_at` 기록
...
- protected root loader·실행 승인·접근 audit 확인
- 승인된 policy 이후 최초 실행
