...
- `true`는 비식별 합성 데이터를 사용하는 Local 환경과 Issue #838 Accepted Decision 및 `EXT-PRIV-003` 승인 범위에서 명시적으로 opt-in한 Production에서 허용한다. Staging에서 활성화하면 거부한다.
...
- 실제 사용자 history 전송의 Production explicit opt-in은 Issue #838과 `EXT-PRIV-003`에서 승인되었다. 기본값은 `false`이며 Staging은 계속 금지되고, Production은 명시적으로 `true`를 설정한 경우에만 활성화된다.
