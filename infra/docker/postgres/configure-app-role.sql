...
SELECT
  ...
  AND bool_and(length(name)>0)
    FILTER (WHERE name NOT IN (:'index_builder_user'))
...
\if :roles_valid
\else
  \echo 'Admin, Migration, Runtime, Writer and optional Index Builder role must be distinct'
...
FROM (VALUES (''), ('IN SCHEMA public')) AS scopes(scope)
CROSS JOIN (VALUES ('TABLES'), ('SEQUENCES')) AS objects(object_type)
CROSS JOIN (VALUES (:'index_builder_user')) AS roles(name)
WHERE length(name)>0
\gexec
COMMIT;
