...
async def test_login_waits_for_concurrent_logout_and_issues_latest_token_version() -> None:
    ...
    발급하는지 검증합니다."""
    user = await _create_committed_user(email=f"login-race-{uuid4().hex[:10]}@test.local")
    ...
    try:
        ...
        # authenticate()가 이 시점에 읽은 것과 같은, 아직 증가 전 값(0)을 가진 User 객체를
        # login()에 그대로 전달합니다.
        assert user.token_version == 0
        login_task = asyncio.create_task(AuthService(UserRepository(login_session)).login(user))
