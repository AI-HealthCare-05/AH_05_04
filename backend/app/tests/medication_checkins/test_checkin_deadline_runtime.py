...
from app.commands import schedule_checkin_deadlines as scheduler
...
async def test_success_logs_counts_and_last_success_in_rendered_message(monkeypatch):
    ...
    monkeypatch.setattr(
        command,
        "generate_unconfirmed_checkins_once",
        AsyncMock(return_value=UnconfirmedGenerationResult(processed_count=3, duplicate_count=1)),
    )
    close = AsyncMock()
    ...
    assert "processed_count=3 duplicate_count=1" in output
...
async def test_failure_logs_only_fixed_reason_and_closes_database(monkeypatch, failure):
    logger = Mock()
    monkeypatch.setattr(command, "default_logger", logger)
    monkeypatch.setattr(command, "generate_unconfirmed_checkins_once", AsyncMock(side_effect=failure))
    close = AsyncMock()
    monkeypatch.setattr(command, "close_database", close)
...
async def test_cancelled_batch_closes_database_and_is_not_reported_as_success(monkeypatch):
    close = AsyncMock()
    monkeypatch.setattr(command, "close_database", close)
    monkeypatch.setattr(command, "generate_unconfirmed_checkins_once", AsyncMock(side_effect=asyncio.CancelledError))
    with pytest.raises(asyncio.CancelledError):
        await command.run()
    close.assert_awaited_once()
