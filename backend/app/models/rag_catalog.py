...
class RagMedicationProduct(Base):
    ...
    aliases: Mapped[list["RagMedicationAlias"]] = relationship(back_populates="product")
    components: Mapped[list["RagMedicationProductComponent"]] = relationship(back_populates="product")
...
class RagMedicationIngredient(Base):
    ...
    source_snapshot: Mapped["RagSourceSnapshot"] = relationship()
    aliases: Mapped[list["RagMedicationAlias"]] = relationship(back_populates="ingredient")
    components: Mapped[list["RagMedicationProductComponent"]] = relationship(back_populates="ingredient")
...
class RagMedicationAlias(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUIDChar(), primary_key=True, default=uuid4)
    source_snapshot_id: Mapped[UUID] = mapped_column(UUIDChar(), ForeignKey("rag_source_snapshot.id"), nullable=False)
    product_id: Mapped[UUID | None] = mapped_column(UUIDChar(), ForeignKey("rag_medication_product.id"), nullable=True)
    ingredient_id: Mapped[UUID | None] = mapped_column(
        UUIDChar(), ForeignKey("rag_medication_ingredient.id"), nullable=True
    )
    target_type: Mapped[RagMedicationAliasTargetType] = mapped_column(String(20), nullable=False)
    ...
    source_snapshot: Mapped["RagSourceSnapshot"] = relationship()
    product: Mapped[RagMedicationProduct | None] = relationship(back_populates="aliases")
    ingredient: Mapped[RagMedicationIngredient | None] = relationship(back_populates="aliases")
...
class RagMedicationProductComponent(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUIDChar(), primary_key=True, default=uuid4)
    source_snapshot_id: Mapped[UUID] = mapped_column(UUIDChar(), ForeignKey("rag_source_snapshot.id"), nullable=False)
    product_id: Mapped[UUID] = mapped_column(UUIDChar(), ForeignKey("rag_medication_product.id"), nullable=False)
    ingredient_id: Mapped[UUID] = mapped_column(UUIDChar(), ForeignKey("rag_medication_ingredient.id"), nullable=False)
    ...
    source_snapshot: Mapped["RagSourceSnapshot"] = relationship()
    product: Mapped[RagMedicationProduct] = relationship(back_populates="components")
    ingredient: Mapped[RagMedicationIngredient] = relationship(back_populates="components")
