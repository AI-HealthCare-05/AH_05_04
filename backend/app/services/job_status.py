...
from app.models.async_jobs import AiJob, AiJobFailureCode, AiJobStatus, AiJobType, DomainType
...
# `available_at`이 지나도 Job은 바로 PROCESSING이 되지 않습니다 — Reconciler 주기 → 새
# Outbox 생성 → Publisher XADD(#219) → Worker lease 획득까지 남아 있고, 그 사이에는
# RETRY_WAIT가 유지됩니다(outbox-stream-v1.md). 그 구간에 Retry-After: 0을 보내면 client가
# 대기 없이 재조회를 반복하므로 최소 1초 하한을 둡니다. Reconciler 실행 주기는 #142에서
# 확정되며, 확정되면 이 상수를 그 값(또는 그 값 기반 최소치)으로 교체해야 합니다.
_MIN_RETRY_AFTER_SECONDS_AFTER_AVAILABLE_AT = 1
...
class JobStatusService:
    ...
    `domain_type`/`domain_id` 조회 방식은 `_resolve_domain_reference()`를 참고하세요.
    ...
    async def get_job_status(self, *, user: User, job_id: UUID) -> JobStatusResult:
        ...
        domain_reference = await self._resolve_domain_reference(job=job)
    ...
    async def rediscover_ocr_job(self, *, user: User, document_id: UUID) -> JobStatusResult:
        ...
        return await self._rediscover_job(
            user=user,
            persistent_job_id=ocr_job.ai_job_id,
            domain_type=DomainType.OCR_JOB,
            domain_id=ocr_job.id,
        )
    ...
    async def _rediscover_job(
        self,
        *,
        user: User,
        persistent_job_id: UUID | None,
        domain_type: DomainType,
        domain_id: UUID,
    ) -> JobStatusResult:
        """`rediscover_ocr_job`/`rediscover_guide_job`이 공유하는 마지막 단계입니다.
        `persistent_job_id`(도메인 row의 영속 `ai_job_id`, 없으면 `None`)가 있으면 그대로
        쓰고, 없으면 `get_latest_job_id_for_domain()`(Outbox 기반 임시 조회, #212 참고)으로
        fallback합니다."""
        job_id = persistent_job_id
        if job_id is None:
            job_id = await self.job_repository.get_latest_job_id_for_domain(
                domain_type=domain_type, domain_id=domain_id
            )
        if job_id is None:
            raise _job_not_found_error()
    ...
    async def _resolve_owned_result_url(
        ...
        *,
        ...
        항상 수행합니다. 도메인 타입별 조회·URL 구성은 각 `_*_result_url()`에 위임합니다."""
        if domain_type is DomainType.OCR_JOB:
            return await self._ocr_result_url(user=user, job=job, domain_id=domain_id)
        if domain_type is DomainType.GUIDE:
            return await self._guide_result_url(user=user, job=job, domain_id=domain_id)
        return await self._chat_result_url(user=user, job=job, domain_id=domain_id)

    async def _ocr_result_url(self, *, user: User, job: AiJob, domain_id: UUID) -> str | None:
...
def _compute_retry_after_seconds(job: AiJob) -> int | None:
    if job.status is not AiJobStatus.RETRY_WAIT:
        return None
    remaining_seconds = round((job.available_at - datetime.now(UTC)).total_seconds())
    if remaining_seconds <= 0:
        return _MIN_RETRY_AFTER_SECONDS_AFTER_AVAILABLE_AT
...
def _build_error(job: AiJob) -> JobErrorData | None:
    ...
    if code is not None and code in _FAILURE_MESSAGES:
        return JobErrorData(code=cast(AiJobFailureCode, code), message=_FAILURE_MESSAGES[code])
    # chk_ai_job_failed_code(FAILED에서 failure_code NOT NULL)와
    # chk_ai_job_failure_code_values(7개 값만 허용) CHECK 제약으로 정상 데이터에서는 오지
    # 않지만, `JobErrorData.code`가 `AiJobFailureCode` Literal이 된 뒤로도(#148 리뷰 지적:
    # OpenAPI가 실제 failure code 집합을 표현해야 함) DTO 계약을 깨지 않기 위해 안전한
    # 값으로 fail-closed합니다.
