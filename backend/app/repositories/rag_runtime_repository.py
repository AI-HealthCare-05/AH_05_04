...
class RagRuntimeRepository:
    ...
    async def get_guide_runtime_request_carrier_by_job(self, ai_job_id: UUID) -> GuideRuntimeRequestCarrier | None:
        execution_context = await self.get_execution_context_by_job(ai_job_id)
        if (
            execution_context is None
            or execution_context.guide_id is None
            or execution_context.chat_message_id is not None
        ):
            return None
        bundle = await self.session.scalar(
            select(RagRuntimeReleaseBundle).where(
                RagRuntimeReleaseBundle.id == execution_context.runtime_release_bundle_id,
                RagRuntimeReleaseBundle.bundle_manifest_hash == execution_context.runtime_release_bundle_manifest_hash,
                RagRuntimeReleaseBundle.execution_manifest_id == execution_context.runtime_execution_manifest_id,
            )
        )
        if bundle is None:
            return None
        bundle_sources = await self.list_bundle_sources(bundle.id)
        identifications = await self.list_execution_identifications(execution_context.id)
        return GuideRuntimeRequestCarrier(
            ...
            runtime_release_bundle_id=execution_context.runtime_release_bundle_id,
            runtime_release_bundle_manifest_hash=execution_context.runtime_release_bundle_manifest_hash,
