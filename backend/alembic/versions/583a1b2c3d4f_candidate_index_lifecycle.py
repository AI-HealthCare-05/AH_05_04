...
LEXICAL_STORAGE_HASH_CHECK = "chk_rag_candidate_index_member_lexical_storage_hash"
...
def _candidate_index_records_exist() -> bool:
    ...
    return bool(
        bind.execute(
            sa.text(f"SELECT EXISTS (SELECT 1 FROM {VERSION_TABLE}) OR EXISTS (SELECT 1 FROM {MEMBER_TABLE})")
        ).scalar()
    )


def upgrade() -> None:
    op.add_column(MEMBER_TABLE, sa.Column("lexical_storage_hash", sa.String(length=64), nullable=True))
    op.add_column(MEMBER_TABLE, sa.Column("embedding_storage_hash", sa.String(length=64), nullable=True))
    op.create_check_constraint(
        LEXICAL_STORAGE_HASH_CHECK,
        MEMBER_TABLE,
...
def downgrade() -> None:
    ...
    op.drop_constraint(LEXICAL_STORAGE_HASH_CHECK, MEMBER_TABLE, type_="check")
    op.drop_column(MEMBER_TABLE, "embedding_storage_hash")
    op.drop_column(MEMBER_TABLE, "lexical_storage_hash")
