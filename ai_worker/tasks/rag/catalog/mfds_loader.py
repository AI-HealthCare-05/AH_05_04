...
async def load_mfds_catalog(
    *,
    ...
    제외가 있으면 부분 build를 만들지 않고 원본 값과 사유를 반환한다.
    ...
    if {p.canonical_code for p in products} != {row.item_seq for row in inspection.observations}:
        raise ValueError("MFDS Loader product scope must match complete detail input")
