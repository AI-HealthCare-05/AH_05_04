...
def build_fixture_evidence() -> tuple[SanitizedFixtureEvidence, ...]:
    fixture_directory = repository_root() / "tests" / "fixtures" / "rag" / "mfds"
    fixture_names = (
        "synthetic_success_page_1.json",
        "synthetic_success_page_2.json",
            ...
            json.dumps(
                {
                    ...
                    "primary_key_validation_passed": (
                        args.full_scan
                        and result.status is SourceRunStatus.SUCCEEDED
                        and result.record_count == total_count
                    ),
